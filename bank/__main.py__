import argparse
import logging
from classes import Account, Employee, Customer, Service, Storage
...
def remove_employee(args):
    ...
    with Storage() as storage:
        try:
            ...
            logging.critical("Could not find an employee with index {0}".format(args.index))
...
def remove_customer(args):
    ...
    with Storage() as storage:
        try:
            ...
            logging.critical("Could not find a customer with index {0}".format(args.index))
